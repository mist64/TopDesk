...
:BitMap	b	$03,$18,$00,$c5,$aa,$aa,$aa,$55
...
.IconTab	b	0,0,0,0	; Anzahl wird berechnet
	w	TrashMap
	b	39-TrashX,191-TrashY,TrashX,TrashY
	w	TrashService
	w	PrintMap
	b	2,193-PrintY,PrintX,PrintY
	w	PrintService
	w	BitMap
	b	STARTA_X,STARTA_Y,BitX,BitY
	w	OpenD8
	w	BitMap
	b	STARTB_X,STARTB_Y,BitX,BitY
	w	OpenD9
	w	BitMap
	b	STARTC_X,STARTC_Y,BitX,BitY
	w	OpenD10
	w	BitMap
	b	STARTD_X,STARTD_Y,BitX,BitY
	w	OpenD11
:OpenD11	lda	#11
	b	$2c
:OpenD10	lda	#10
...
::t2	b	"close all",GOTOX
	w	WINDOWRIGHT-22
	b	128,BOLDON,"C",PLAINTEXT,0
::t3	b	"select contents",GOTOX
	w	WINDOWRIGHT-22
	b	128,BOLDON,"W",PLAINTEXT,0
::t4	b	"select certain files",GOTOX
	w	WINDOWRIGHT-22
...
::t1	b	"new folder",GOTOX
	w	SPCRIGHT-20
	b	128,BOLDON,"O",PLAINTEXT,0
::t2	b	"set clock",GOTOX
	w	SPCRIGHT-20
...
:AutoSwapFlag	b	"  automatic swap",0
...
::t1	b	"DeskTop not present on",0
::t2	b	"RAM-Disks A or B!",0
...
:GetStartPos	ldx	#00
	...
	sta	r2H,x
	rts
::xL	b	<(STARTA_X*8),<(STARTB_X*8),<(STARTC_X*8),<(STARTD_X*8)
::xH	b	>(STARTA_X*8),>(STARTB_X*8),>(STARTC_X*8),>(STARTD_X*8)
::y	b	STARTA_Y,STARTB_Y,STARTC_Y,STARTD_Y
...
::09	jsr	GetWorkArea
	...
	jsr	FehlerAusgabe2
::e10	rts
::tab1	w	0,(3*MOVE_OFFS),MOVE_OFFS
	w	-(3*MOVE_OFFS),-(3*MOVE_OFFS),-MOVE_OFFS
::10	jsr	MyCheckFiles
	txa
	bmi	:20
	pha
...
:NoMultiFileBox	b	$81
	...
	b	OK,17,72
	b	NULL
::t1	b	BOLDON,"This function can not be",0
::t2	b	"carried out under",0
::t3	b	"Multi-File",0
...
::db	b	$81
	...
	w	:t2
	b	OK,17,72,NULL
::t1	b	"This programme only works",0
::t2	b	"together with GEOS 64.",0
::db2	b	$81
	b	$0b,$10,$10
	w	:t1b
	b	$0b,$10,$20
	w	:t2b
	b	OK,17,72,NULL
::t1b	b	"A Programme start from either drive",0
::t2b	b	"C or D is not possible.",0
:activeFile	b	$ff
